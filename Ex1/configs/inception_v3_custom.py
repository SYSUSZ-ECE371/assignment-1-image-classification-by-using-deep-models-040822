...
model = dict(
    ...
    backbone=dict(
        num_classes=5, # Ex1:变成5分类模型 , inception_v3的num_classes在backbone里，比较特殊。
        init_cfg=dict(
            type='Pretrained', # Ex1：启动预训练
            checkpoint='https://download.openmmlab.com/mmclassification/v0/inception-v3/inception-v3_3rdparty_8xb32_in1k_20220615-dcd4d910.pth',
            prefix='backbone',
            
        )),
    head=dict(
        topk=(1),      # Ex1:只使用Topk1的训练效果似乎不好。
)
)
